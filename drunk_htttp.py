...
class randomHttp(object):
  ...
  def makeData(self, max_size=8000):
    return {'data': '%01x' % random.randrange(0, 16**random.randrange(0, max_size))}

  def makeRcode(self):
    rcode = random.choice(self.status_codes.keys())
    return {'rcode': str(rcode), 'status': self.status_codes[rcode]}

  def makeSpecificHeads(self, key='Example', value='Mozilla/4.0'):
  ...
  def requestObject(self):
    ...
    req['headers'].update(self.makeSpecificHeads( key='Host', value='example.example' ))
    return req
  ...
  def responseObject(self):
    ...
    resp['headers'].update(self.makeSpecificHeads( key='Server', value='Example v1.0' ))
    return resp
